...
class UIController {
  ...
  initObservers() {
    ...
    if (document.getElementById("sound-observer").checked) {
      this.taskSubject.addObserver(
        new SoundObserver(this.addNotification.bind(this))
      );
    }
  }
  ...
  applyDecorators() {
    ...
    // Aplicar decorador de checklist, se selecionado
    if (document.getElementById("checklist").checked) {
      const itemsRaw = document.getElementById("checklist-items").value;
      const items = itemsRaw
        .split("\n")
        .map((i) => i.trim())
        .filter((i) => i);
      if (items.length > 0) {
        task = new ChecklistDecorator(task, items);
      }
    }
